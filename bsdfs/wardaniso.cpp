...
public:
	...
	WardAniso(const Properties &props)
		: BSDF(props) {
		...
		Float alpha = props.getFloat("alpha", 0.1f),
			  ...
			  rotate = props.getFloat("rotate", 0.0f);

		m_rotate = new ConstantFloatTexture(rotate);
	...
	WardAniso(Stream *stream, InstanceManager *manager)
	 : BSDF(stream, manager) {
		...
		m_rotate = static_cast<Texture *>(manager->getInstance(stream));
		configure();
	}
	...
	Spectrum eval(const BSDFSamplingRecord &bRec, EMeasure measure) const {
		...
		if (hasSpecular) {
			...
			Float rotate = m_rotate->eval(bRec.its).average();
			const float cos_phi = std::cosf(rotate * M_PI);
			const float sin_phi = std::sinf(rotate * M_PI);
			Float alphaUTmp = std::abs(cos_phi * alphaU - sin_phi * alphaV);
			alphaV = std::abs(sin_phi * alphaU + cos_phi * alphaV);
			alphaU = alphaUTmp;
	...
	Spectrum sample(BSDFSamplingRecord &bRec, const Point2 &sample) const {
		Float pdf;
		return WardAniso::sample(bRec, pdf, sample);
	}
	...
	void addChild(const std::string &name, ConfigurableObject *child) {
		if (child->getClass()->derivesFrom(MTS_CLASS(Texture))) {
			if (name == "alphaU")
				...
				m_rotate = static_cast<Texture *>(child);
	...
	void serialize(Stream *stream, InstanceManager *manager) const {
		...
		manager->serialize(stream, m_rotate.get());
	}
	...
	std::string toString() const {
		...
		oss << "  diffuseReflectance = " << indent(m_diffuseReflectance->toString()) << "," << endl
			...
			<< "  rotate = " << indent(m_rotate->toString()) << endl
 ...
 */
class WardAnisoShader : public Shader {
public:
	WardAnisoShader(Renderer *renderer,
			const Texture *diffuseColor,
			const Texture *specularColor,
			const Texture *alphaU,
			const Texture *alphaV,
			const Texture *rotate) : Shader(renderer, EBSDFShader),
			m_diffuseReflectance(diffuseColor),
			m_specularReflectance(specularColor),
			m_alphaU(alphaU), m_alphaV(alphaV), m_rotate(rotate) {
		...
		m_rotateShader = renderer->registerShaderForResource(m_rotate.get());
	}
	...
	void putDependencies(std::vector<Shader *> &deps) {
		...
		deps.push_back(m_rotateShader.get());
	}
	...
	void cleanup(Renderer *renderer) {
		...
		renderer->unregisterShaderForResource(m_rotate.get());
	}
	...
	void generateCode(std::ostringstream &oss,
		...
		oss << "vec3 " << evalName << "(vec2 uv, vec3 wi, vec3 wo) {" << endl
			...
			<< "    float rotate = " << depNames[4] << "(uv)[0];" << endl
			<< "    float cosr = cos(rotate*pi);" << endl
			<< "    float sinr = sin(rotate*pi);" << endl
			<< "    float alphaUTmp = abs(cosr * alphaU + sinr * alphaV);" << endl
			<< "    alphaV = abs(cosr * alphaV - sinr * alphaU);" << endl
			<< "    alphaU = alphaUTmp;" << endl
...
private:
	...
	ref<const Texture> m_rotate;

	ref<Shader> m_diffuseReflectanceShader;
	ref<Shader> m_specularReflectanceShader;
	ref<Shader> m_alphaUShader;
	ref<Shader> m_alphaVShader;
	ref<Shader> m_rotateShader;
};

Shader *WardAniso::createShader(Renderer *renderer) const {
	return new WardAnisoShader(renderer, m_diffuseReflectance.get(),
		m_specularReflectance.get(), m_alphaU.get(), m_alphaV.get(), m_rotate.get());
}

MTS_IMPLEMENT_CLASS(WardAnisoShader, false, Shader)
MTS_IMPLEMENT_CLASS_S(WardAniso, false, BSDF);
MTS_EXPORT_PLUGIN(WardAniso, "Anisotropic Ward BRDF");
MTS_NAMESPACE_END
